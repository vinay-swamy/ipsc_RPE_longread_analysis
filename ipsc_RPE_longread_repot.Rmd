...
ref_tx_bed <-  ref_gtf %>% filter(type == 'transcript') %>% mutate(score=100,start = start-5000, end = end + 5000) %>% select(seqid, start, end, transcript_id, score, strand) %>% 
  from_data_frame %>% 
  RBedtools('sort', i=.)
...
novel_loci_bed <- talon_sr_lr_pass_ttab %>% 
 ...
 RBedtools('intersect', options = '-s -v', a=novel_loci_bed, b=ref_tx_bed) %>% to_data_frame %>% View
